...
LUA_API int lua_absindex (lua_State *L, int idx) {
  return (idx > 0 || ispseudo(idx)) //如果是大于0的索引，或者是伪索引
         ? idx
         : cast_int(L->top - L->ci->func + idx);
         //如果是小于0的索引，获取第一个空白-函数的偏移量+idx,则计算出的是一个正数的idx
}

//lua_gettop(L) 可以返回函数收到的参数个数
//TOP为第一个空闲的slot,L->ci->func为当前运行的函数的函数名次的索引
LUA_API int lua_gettop (lua_State *L) {
  return cast_int(L->top - (L->ci->func + 1));
}

//设置L的当前函数调用的栈的大小为idx,并把新增的参数设置为nil，然后把top设置为栈顶
//如果idx<0,则减少top
...
//不要用伪索引来调用这个函数， 因为伪索引不是真正指向堆栈上的位
...
LUA_API void lua_rawsetp (lua_State *L, int idx, const void *p) {
  ...
  t = index2addr(L, idx);
  api_check(L, ttistable(t), "table expected");
  setpvalue(&k, cast(void *, p));//对k进行赋值
...
//获取第 n 个 upvalue ， 把这个 upvalue 的值压入堆栈，并且返回它的名字。 
//funcindex 指向堆栈上 closure 的位置。
...
//设置 closure 的 upvalue 的值。 它把栈顶的值弹出并赋于 upvalue 并返回 upvalue 的名字。
